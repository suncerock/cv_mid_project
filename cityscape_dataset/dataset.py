...
class cityscapesLoader(data.Dataset):
    ...
    def transform(self, img, lbl):
        ...
        """
        img = np.array(
            Image.fromarray(img).resize((self.img_size[0], self.img_size[1]))
        )  # uint8 with RGB mode
        img = img[:, :, ::-1]  # RGB -> BGR
        ...
        lbl = np.array(Image.fromarray(lbl).convert("F").resize((self.img_size[0], self.img_size[1]), resample=Image.NEAREST))
        lbl = lbl.astype(int)

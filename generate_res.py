...
model.load_state_dict(torch.load(r'./pretrained_model/FCN_ResNet_32x_pretrained_backbone_Cityscapes', map_location='cpu'))
model.to(device)

test_img_path = []
for root, dirnames, file in os.walk(r'./data/Cityscapes/leftImg8bit/val'):
    for f in file:
...
for i in range(len(test_img_path)):
    ...
    print("({:3d}/{:d}) Processing {:s}".format(i+1, len(test_img_path)+1, img_path), end='')
    sys.stdout.flush()
    ...
    with torch.no_grad():
        ...
        plt.axis(False)
        plt.savefig(os.path.join(r'./data/Cityscapes/colors', img_name.replace('_leftImg8bit', '')+'img'), format='png')
        plt.clf()